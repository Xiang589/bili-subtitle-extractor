...
基础环境要求：Chrome 116+。扩展默认最大录音时长为 10 分钟；本地 ASR 服务默认最大上传体积为 200MB，可通过 `ASR_MAX_UPLOAD_MB` 调整。
...
1. 启动扩展录音并保持超过 10 分钟。
2. 预期：录音达到默认上限后自动停止，并继续上传到本地 ASR 服务。
3. 设置较小上传限制，例如 `$env:ASR_MAX_UPLOAD_MB="1"`。
4. 录制超过该限制的音频并停止。
5. 预期：本地服务返回上传过大提示，扩展不崩溃并展示友好错误。

## 12. popup 状态机和重置

1. 连续快速点击“开始录音”。
2. 预期：只创建一个 tab 音频 stream，不会重复捕获当前标签页。
3. 录制中再次打开 popup。
4. 预期：popup 仍显示“录制中”，开始按钮禁用，停止按钮启用。
5. 点击“停止”后观察 stopping/uploading 阶段。
6. 预期：正在停止录音和正在生成字幕时，开始按钮与停止按钮都禁用。
7. 模拟或触发错误状态。
8. 预期：popup 显示错误信息，允许点击“重置状态”。
9. 出现 “Cannot capture a tab with an active stream.” 或残留 active stream 提示后，点击“重置状态”，再重新开始录音。
10. 预期：重置后可重新录制，不生成空文件，不崩溃。
11. 录制时关闭 popup，再重新打开。
12. 预期：popup 从 service worker 读取真实状态，仍与实际录音状态一致。
