...
不会。v0.1.1 只处理已有字幕，不做 ASR、OCR 或画面识别。

**需要登录才能看的字幕怎么办？**

脚本不会发送 Cookie，也不会绕过登录、付费、会员、地区或权限限制。若字幕接口要求登录或权限校验，脚本会提示后停止。

本脚本默认不发送 Cookie，这是隐私设计。需要登录、会员、付费、地区或其他权限校验的字幕不会被读取。

**多 P 视频会怎么处理？**

v0.1.1 只处理用户当前主动打开的视频页和当前分 P。URL 中 `p=2` 时会尝试读取 P2 对应的 CID 并导出 P2 字幕。
